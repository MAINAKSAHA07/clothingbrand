...
import state, { undo, historyStack } from '../store';
...
const Customizer = () => {
  ...
  const snapHistory = useSnapshot(historyStack);
    ...
    <AnimatePresence>
      {!snap.intro && (
        <>
          ...
          <motion.div
            className="absolute z-10 top-5 right-5 flex items-center gap-3"
            {...fadeAnimation}
          >
            <button
              onClick={undo}
              disabled={!snapHistory.canUndo}
              className={`px-4 py-2 text-xs font-bold rounded-lg border shadow-sm transition-all duration-200 ${
                snapHistory.canUndo
